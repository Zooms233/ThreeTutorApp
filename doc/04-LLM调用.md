...
- 教材：`TEXTBOOK/*.md`——教学与问答注入当前节全文（节标题到下一个同级标题之间，超 8000 字截断）+ 教材目录（全部节标题清单）；聊天/群聊不注入
- 对话：`CHAT/第N课.jsonl` 全部 message 行——meta 行不映射；user→user、tutor→assistant 原文原样；用户消息 time 并入 content 头部；连续 user 消息合并；全文件历史（含文件头交流段）；textbook 指针行读盘物化为 user 消息（兼容旧档）；auto 标记（群聊）仅 social 场景使用，qa 的写入目标文件天然不含群聊
