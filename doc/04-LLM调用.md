...
- 教学（ongoing）：本课导师（meta.tutor）回应——教学规则 + 本课导师档案 + 学习者档案 + STATE + PROGRESS + 教学范围（大纲，若有）+ 今日日期 + 材料（当前位置内容+目录）+ 对话 + `read` 工具（按需查阅，如温习前节/预习后节）
...
**UI**：翻书期间 busy 文案切「{导师} 正在翻阅教材…」；`tool` 行渲染为系统提示「📖 {导师} 查阅材料：path 第 N 行起」，`tool_call` 行不渲染。
